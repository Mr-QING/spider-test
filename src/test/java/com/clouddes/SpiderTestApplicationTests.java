...
@SpringBootTest
public class SpiderTestApplicationTests implements PageProcessor{
...
//            File file = new File("D:\\workspace\\java\\WebAnimationTest\\src\\main\\java\\Webmagic\\AutoHome\\origit_data\\car_oil.txt");//进口，出口
			FileUtils.writeLines(file, one_car, null);
		} catch (Exception e) {
		}
	...
	public void brand_model(){
		try {
			...
			for(String list_car :list){
				...
				Spider.create(new SpiderTestApplicationTests()).addUrl(https).thread(5).run();
